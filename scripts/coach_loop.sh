...
COACH_LOOP_ENABLED="${COACH_LOOP_ENABLED:-false}"
...
log "Coach API enabled: ${COACH_LOOP_ENABLED}"
...
while true; do
    ...
    if [ "$COACH_LOOP_ENABLED" != "true" ]; then
        log "Coach API disabled; skipping coach audit cycle"
    elif (
        cd "$REPO_DIR" &&
